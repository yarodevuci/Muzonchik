...
extension ARNTransitionAnimator {
    ...
    internal func handlePan(_ recognizer: UIPanGestureRecognizer) {
        ...
        if recognizer.state == .began {
            ...
            case .bottom:
                animationRatio = 0 //(location.y - self.panLocationStart) / bounds.height
            case .left:
...
extension ARNTransitionAnimator: UIViewControllerTransitioningDelegate {
    ...
    public func interactionControllerForPresentation(using animator: UIViewControllerAnimatedTransitioning) -> UIViewControllerInteractiveTransitioning? {
        if self.gesture != nil && self.interactiveType == .present {
            self.isPresenting = true
            return self
        }
        return nil
    ...
    public func interactionControllerForDismissal(using animator: UIViewControllerAnimatedTransitioning) -> UIViewControllerInteractiveTransitioning? {
        if self.gesture != nil && self.interactiveType == .dismiss {
            self.isPresenting = false
            return self
        }
        return nil
